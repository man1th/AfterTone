...
const NavBtn: Component<{
  ...
  <button
    ...
    style={{
      ...
      background: props.active ? "#2a2a2a" : "transparent",
      border: "none",
      display: "flex",
...
const ColorWheelControl: Component<{
  h: number;
  s: number;
  disabled: boolean;
  ...
  const outerPos = () => {
    const rad = (props.h * Math.PI) / 180;
    return {
      x: CENTER + R_OUTER * Math.cos(rad),
      y: CENTER + R_OUTER * Math.sin(rad),
    };
  };
      ...
      >
        ...
        <div
          style={{
            ...
            "border-radius": "50%",
            background:
              "radial-gradient(circle, #808080 0%, rgba(128,128,128,0) 65%), conic-gradient(from 90deg, red, yellow, lime, cyan, blue, magenta, red)",
...
const ColorGradingPanel: Component<{
  state: any;
  bypassed: boolean;
  update: (field: string, val: number) => void;
}> = (props) => {
  const [activeTab, setActiveTab] = createSignal<"s" | "m" | "h" | "g">("s");
  const tabs = [
    { id: "s", label: "Shadows" },
    { id: "m", label: "Midtones" },
    { id: "h", label: "Highlights" },
    { id: "g", label: "Global" },
  ];
  return (
    <div style={{ padding: "12px 14px" }}>
      ...
      <ColorWheelControl
        ...
        onChange={(h, s) => {
          props.update(`cg_${activeTab()}_h`, h);
          props.update(`cg_${activeTab()}_s`, s);
        }}
      />
      <div style={{ display: "flex", "flex-direction": "column", gap: "6px" }}>
        <Slider
          label="Hue"
          value={props.state[`cg_${activeTab()}_h`]}
          min={0}
          max={360}
          disabled={props.bypassed}
        ...
        <Slider
          label="Saturation"
          value={props.state[`cg_${activeTab()}_s`]}
          min={0}
          max={100}
          disabled={props.bypassed}
        ...
        <Slider
          label="Luminance"
          value={props.state[`cg_${activeTab()}_l`]}
          min={-100}
          max={100}
          disabled={props.bypassed}
...
const MIXER_COLORS = [
  {
    ...
    label: "Red",
    hex: "#ef4444",
    hGrad: "linear-gradient(to right, #ec4899, #ef4444, #f97316)",
  },
  {
    id: "o",
    label: "Orange",
    hex: "#f97316",
    hGrad: "linear-gradient(to right, #ef4444, #f97316, #eab308)",
  },
  {
    id: "y",
    label: "Yellow",
    hex: "#eab308",
    hGrad: "linear-gradient(to right, #f97316, #eab308, #22c55e)",
  },
  {
    id: "g",
    label: "Green",
    hex: "#22c55e",
    hGrad: "linear-gradient(to right, #eab308, #22c55e, #06b6d4)",
  },
  {
    id: "a",
    label: "Aqua",
    hex: "#06b6d4",
    hGrad: "linear-gradient(to right, #22c55e, #06b6d4, #3b82f6)",
  },
  {
    id: "b",
    label: "Blue",
    hex: "#3b82f6",
    hGrad: "linear-gradient(to right, #06b6d4, #3b82f6, #a855f7)",
  },
  {
    id: "p",
    label: "Purple",
    hex: "#a855f7",
    hGrad: "linear-gradient(to right, #3b82f6, #a855f7, #ec4899)",
  },
  {
    id: "m",
    label: "Magenta",
    hex: "#ec4899",
    hGrad: "linear-gradient(to right, #a855f7, #ec4899, #ef4444)",
  },
];

const RadioBtn: Component<{
  active: boolean;
  label: string;
  onClick: () => void;
}> = (props) => (
  ...
  >
    <div
      style={{
        ...
        height: "12px",
        "border-radius": "50%",
        border: props.active ? "4px solid #aaa" : "1px solid #555",
        transition: "all 0.15s",
...
const ColorMixerPanel: Component<{
  state: any;
  bypassed: boolean;
  update: (field: string, val: number) => void;
}> = (props) => {
            ...
            (c) =>
              activeColor() === c.id && (
                ...
                >
                  <Slider
                    label="Hue"
                    value={props.state[`cm_h_${c.id}`]}
                    disabled={props.bypassed}
                    onChange={(v) => props.update(`cm_h_${c.id}`, v)}
                    trackBg={c.hGrad}
                  />
                  <Slider
                    label="Saturation"
...
const App: Component = () => {
  ...
  const [activeSliderName, setActiveSliderName] = createSignal<string | null>(
    null,
  );
  ...
  const getProcessedLightState = () => ({
    ...
    hal_thresh: bypassed.halation ? 80 : lightState.hal_thresh,
    hal_radius: bypassed.halation ? 0 : lightState.hal_radius,
    hal_intensity: bypassed.halation ? 0 : lightState.hal_intensity,
    hal_color: lightState.hal_color,
    show_hal_map: lightState.show_hal_map,
    bloom_intensity: bypassed.bloom ? 0 : lightState.bloom_intensity,
    grain_amount: bypassed.grain ? 0 : lightState.grain_amount,
    grain_size: lightState.grain_size,
    grain_roughness: lightState.grain_roughness,
    grain_color_variance: lightState.grain_color_variance,
    cg_s_h: bypassed.color_grading ? 0 : lightState.cg_s_h,
    cg_s_s: bypassed.color_grading ? 0 : lightState.cg_s_s,
    cg_s_l: bypassed.color_grading ? 0 : lightState.cg_s_l,
    cg_m_h: bypassed.color_grading ? 0 : lightState.cg_m_h,
    cg_m_s: bypassed.color_grading ? 0 : lightState.cg_m_s,
    cg_m_l: bypassed.color_grading ? 0 : lightState.cg_m_l,
    cg_h_h: bypassed.color_grading ? 0 : lightState.cg_h_h,
    cg_h_s: bypassed.color_grading ? 0 : lightState.cg_h_s,
    cg_h_l: bypassed.color_grading ? 0 : lightState.cg_h_l,
    cg_g_h: bypassed.color_grading ? 0 : lightState.cg_g_h,
    cg_g_s: bypassed.color_grading ? 0 : lightState.cg_g_s,
    cg_g_l: bypassed.color_grading ? 0 : lightState.cg_g_l,
    cm_h_r: bypassed.color_mixer ? 0 : lightState.cm_h_r,
    cm_s_r: bypassed.color_mixer ? 0 : lightState.cm_s_r,
    cm_l_r: bypassed.color_mixer ? 0 : lightState.cm_l_r,
    cm_h_o: bypassed.color_mixer ? 0 : lightState.cm_h_o,
    cm_s_o: bypassed.color_mixer ? 0 : lightState.cm_s_o,
    cm_l_o: bypassed.color_mixer ? 0 : lightState.cm_l_o,
    cm_h_y: bypassed.color_mixer ? 0 : lightState.cm_h_y,
    cm_s_y: bypassed.color_mixer ? 0 : lightState.cm_s_y,
    cm_l_y: bypassed.color_mixer ? 0 : lightState.cm_l_y,
    cm_h_g: bypassed.color_mixer ? 0 : lightState.cm_h_g,
    cm_s_g: bypassed.color_mixer ? 0 : lightState.cm_s_g,
    cm_l_g: bypassed.color_mixer ? 0 : lightState.cm_l_g,
    cm_h_a: bypassed.color_mixer ? 0 : lightState.cm_h_a,
    cm_s_a: bypassed.color_mixer ? 0 : lightState.cm_s_a,
    cm_l_a: bypassed.color_mixer ? 0 : lightState.cm_l_a,
    cm_h_b: bypassed.color_mixer ? 0 : lightState.cm_h_b,
    cm_s_b: bypassed.color_mixer ? 0 : lightState.cm_s_b,
    cm_l_b: bypassed.color_mixer ? 0 : lightState.cm_l_b,
    cm_h_p: bypassed.color_mixer ? 0 : lightState.cm_h_p,
    cm_s_p: bypassed.color_mixer ? 0 : lightState.cm_s_p,
    cm_l_p: bypassed.color_mixer ? 0 : lightState.cm_l_p,
    cm_h_m: bypassed.color_mixer ? 0 : lightState.cm_h_m,
    cm_s_m: bypassed.color_mixer ? 0 : lightState.cm_s_m,
    cm_l_m: bypassed.color_mixer ? 0 : lightState.cm_l_m,
    vig_amount: bypassed.vignette ? 0 : lightState.vig_amount,
    vig_midpoint: bypassed.vignette ? 50 : lightState.vig_midpoint,
    vig_roundness: bypassed.vignette ? 0 : lightState.vig_roundness,
    vig_feather: bypassed.vignette ? 50 : lightState.vig_feather,
  ...
  > = {
    ...
    halation: {
      ...
      features: true,
      reset: () =>
        setLightState({ hal_thresh: 85, hal_radius: 20, hal_intensity: 0 }),
    },
    bloom: {
      title: "Bloom",
      features: true,
      reset: () => setLightState({ bloom_intensity: 0 }),
    },
    grain: {
      title: "Film Grain",
      features: true,
    ...
    switch (id) {
        ...
        return (
          <div style={{ padding: "16px 14px" }}>
            <Histogram
              ...
              activeSlider={
                activeSliderName()
                  ? { name: activeSliderName()!, value: getActiveSliderVal()! }
                  : null
              }
            />
          </div>
      ...
      case "exposure":
        return (
          ...
          >
            <div
              onMouseEnter={() => setActiveSliderName("Exposure")}
              onMouseLeave={() => setActiveSliderName(null)}
            >
              <Slider
                label="Exposure"
      ...
      case "hdr":
        return (
          ...
          >
            <div
              onMouseEnter={() => setActiveSliderName("Highlights")}
              onMouseLeave={() => setActiveSliderName(null)}
            >
              <Slider
                label="Highlights"
            ...
            <div
              onMouseEnter={() => setActiveSliderName("Shadows")}
              onMouseLeave={() => setActiveSliderName(null)}
            >
              <Slider
                label="Shadows"
      ...
      case "vignette":
        return (
          ...
          >
            <Slider
              label="Amount"
              value={lightState.vig_amount}
              min={-100}
              max={100}
              disabled={bypassed.vignette}
              onChange={(v) => setLightState("vig_amount", v)}
            />
            <Slider
              label="Midpoint"
              value={lightState.vig_midpoint}
              min={0}
              max={100}
              disabled={bypassed.vignette}
              onChange={(v) => setLightState("vig_midpoint", v)}
            />
            <Slider
              label="Roundness"
              value={lightState.vig_roundness}
              min={-100}
              max={100}
              disabled={bypassed.vignette}
              onChange={(v) => setLightState("vig_roundness", v)}
            />
            <Slider
              label="Feather"
              value={lightState.vig_feather}
              min={0}
              max={100}
              disabled={bypassed.vignette}
              onChange={(v) => setLightState("vig_feather", v)}
            />
          </div>
        );
      case "bloom":
        return (
          ...
          >
            <Slider
              label="Intensity"
              value={lightState.bloom_intensity}
              min={0}
              max={100}
              disabled={bypassed.bloom}
              onChange={(v) => setLightState("bloom_intensity", v)}
            />
          </div>
        );
      case "color_grading":
        return (
          <ColorGradingPanel
            state={lightState}
            bypassed={bypassed.color_grading}
            update={setLightState as any}
          />
        );
      case "color_mixer":
        return (
        ...
        return (
          ...
          >
            ...
            >
              <div
                style={{ flex: 1 }}
                onMouseEnter={() => setActiveSliderName("Threshold")}
                onMouseLeave={() => setActiveSliderName(null)}
              >
                <Slider
                  label="Threshold"
                  value={lightState.hal_thresh}
                  min={0}
                  max={100}
                  disabled={bypassed.halation}
                  onChange={(v) => setLightState("hal_thresh", v)}
                />
              </div>
              <button
                onClick={() =>
                  setLightState("show_hal_map", !lightState.show_hal_map)
                }
                disabled={bypassed.halation}
                title="Toggle Binary View"
                style={{
                  background: lightState.show_hal_map ? "#e0e0e0" : "#222",
                  color: lightState.show_hal_map ? "#111" : "#aaa",
                  border: "1px solid #444",
                  "border-radius": "4px",
                  padding: "0 6px",
                  "font-size": "9px",
                  "font-weight": "800",
                  cursor: bypassed.halation ? "default" : "pointer",
                  height: "20px",
                  "margin-bottom": "6px",
                  transition: "all 0.15s",
                }}
            ...
            <div
              onMouseEnter={() => setActiveSliderName("Gaussian Blur (px)")}
              onMouseLeave={() => setActiveSliderName(null)}
            >
              <Slider
                label="Radius (px)"
                value={lightState.hal_radius}
                min={0}
                max={100}
                disabled={bypassed.halation}
                onChange={(v) => setLightState("hal_radius", v)}
              />
            </div>
            <Slider
              label="Intensity"
              value={lightState.hal_intensity}
              min={0}
              max={100}
              disabled={bypassed.halation}
              onChange={(v) => setLightState("hal_intensity", v)}
            />
            <div
              style={{
                display: "flex",
                ...
                padding: "6px 0",
                "margin-top": "4px",
                opacity: bypassed.halation ? 0.4 : 1,
              }}
            >
              <span
                style={{
              ...
              <input
                type="color"
                value={lightState.hal_color}
                disabled={bypassed.halation}
                onInput={(e) =>
                  setLightState("hal_color", e.currentTarget.value)
                }
                style={{
                  background: "none",
                  border: "1px solid #444",
      ...
      case "grain":
        return (
          ...
          >
            <Slider
              label="Amount"
              value={lightState.grain_amount}
              min={0}
              max={100}
              disabled={bypassed.grain}
              onChange={(v) => setLightState("grain_amount", v)}
            />
            <Slider
              label="Size"
              value={lightState.grain_size}
              min={0}
              max={100}
              disabled={bypassed.grain}
              onChange={(v) => setLightState("grain_size", v)}
            />
            <Slider
              label="Roughness"
              value={lightState.grain_roughness}
              min={0}
              max={100}
              disabled={bypassed.grain}
              onChange={(v) => setLightState("grain_roughness", v)}
            />
            <Slider
              label="Color Variance"
              value={lightState.grain_color_variance}
              min={0}
              max={100}
              disabled={bypassed.grain}
              onChange={(v) => setLightState("grain_color_variance", v)}
            />
          </div>
        );
    }
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: "2px", "align-items": "center" }}>
          ...
          ></div>
          <NavBtn
            icon="/assets/icons/original.svg"
            label="Reset"
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <main style={{ flex: 1, background: "transparent" }}>
          <Viewport
            lightState={
              isOriginal()
                ? { ...getProcessedLightState(), enabled: false }
                : getProcessedLightState()
            }
            curves={
              !bypassed.curve && !isOriginal() ? curves() : defaultCurves()
            }
            isCompare={isCompare()}
