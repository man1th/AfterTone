    ...
    <div style={{ display: 'flex', 'flex-direction': 'column', height: '100vh', width: '100vw' }}>
      ...
      <header style={{ height: '44px', background: '#1c1c1c', 'border-bottom': '1px solid #282828', display: 'flex', 'align-items': 'center', 'justify-content': 'space-between', padding: '0 16px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', 'align-items': 'center' }}>
          <span style={{ 'font-size': '10px', color: isWasmReady() ? '#4ade80' : '#f87171', 'text-transform': 'uppercase' }}>{isWasmReady() ? 'Core Online' : 'Booting...'}</span>
          <button onClick={() => triggerExport()} style={{ background: '#333', color: '#e0e0e0', border: '1px solid #444', padding: '4px 14px', 'border-radius': '4px', cursor: 'pointer', 'font-size': '11px' }}>Export</button>
        </div>
      </header>

      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        <main style={{ flex: 1, background: '#111' }}><Viewport lightState={getProcessedLightState()} curves={!bypassed.curve ? curves() : defaultCurves()} onHistogramUpdate={setHistogramData} onHoverLuminance={setHoverLuminance} onMetadataUpdate={setMetadata} getExportFn={(fn) => triggerExport = fn} /></main>
            ...
            {(id, index) => (
              <div onDragOver={(e) => { e.preventDefault(); const c = draggedIndex(); if (c !== null && c !== index()) { const o = [...panelOrder()]; o.splice(index(), 0, o.splice(c, 1)[0]); setPanelOrder(o); setDraggedIndex(index()); } }} style={{ opacity: draggedIndex() === index() ? 0.3 : 1 }}>
                <div style={{ display: 'flex', 'align-items': 'center', 'justify-content': 'space-between', padding: '8px 14px', background: '#1e1e1e', 'border-bottom': '1px solid #282828', cursor: 'pointer' }} onClick={() => setExpanded(id, !expanded[id])}>
                  <div style={{ display: 'flex', 'align-items': 'center', gap: '6px' }}><ChevronRight size={14} color="#888" style={{ transform: expanded[id] ? 'rotate(90deg)' : 'rotate(0deg)' }} /><span style={{ 'font-size': '11px', color: '#AAAAAA' }}>{panelMeta[id].title}</span></div>
                  <div style={{ display: 'flex', 'align-items': 'center', gap: '8px' }}>
                    {panelMeta[id].features && (
                      <><button onClick={(e) => { e.stopPropagation(); setBypassed(id, !bypassed[id]); }} style={{ background: 'none', border: 'none', color: bypassed[id] ? '#444' : '#aaa' }}>{bypassed[id] ? <EyeOff size={14} /> : <Eye size={14} />}</button><button onClick={(e) => { e.stopPropagation(); panelMeta[id].reset(); }} style={{ background: 'none', border: 'none', color: '#777' }}><RotateCcw size={12} /></button></>
                    )}
                    <div draggable="true" onDragStart={() => setDraggedIndex(index())} onDragEnd={() => setDraggedIndex(null)} style={{ cursor: 'grab', color: '#555' }}><GripHorizontal size={14} /></div>
                  </div>
                </div>
