...
type PanelId = "histogram" | "crop" | "wb" | "exposure" | "hdr" | "clarity" | "dehaze" | "vignette" | "curve" | "texture" | "bloom" | "halation" | "grain" | "color_grading" | "color_mixer";
...
const ColorWheelControl: Component<{ h: number; s: number; disabled: boolean; onChange: (h: number, s: number) => void; }> = (props) => {
  let wheelRef!: HTMLDivElement; let dragMode: "inner" | "outer" | null = null;
  const handleDrag = (e: any) => {
    ...
    if (dragMode === "outer") { props.onChange(Math.round(angle), props.s); } else { const maxR = 60; const r = Math.min(maxR, Math.sqrt(dx * dx + dy * dy)); props.onChange(Math.round(angle), Math.round((r / maxR) * 100)); }
  };
  ...
  return (
    <div style={{ padding: "12px 14px" }}>
      <div style={{ display: "flex", "border-bottom": "1px solid #333", "margin-bottom": "8px" }}>
        {tabs.map((t) => ( <button onClick={() => setActiveTab(t.id as any)} style={{ flex: 1, background: "none", border: "none", "border-bottom": activeTab() === t.id ? "2px solid #aaa" : "2px solid transparent", color: activeTab() === t.id ? "#ddd" : "#666", "font-size": "10px", "font-weight": "400", padding: "6px 0", cursor: "pointer", transition: "all 0.15s" }}>{t.label}</button> ))}
      </div>
      ...
      <div style={{ display: "flex", "flex-direction": "column", gap: "6px" }}>
        <Slider label="Hue" value={props.state[`cg_${activeTab()}_h`]} min={0} max={360} disabled={props.bypassed} onChange={(v) => props.update(`cg_${activeTab()}_h`, Math.round(v))} trackBg="linear-gradient(to right, red, yellow, lime, cyan, blue, magenta, red)" />
        <Slider label="Saturation" value={props.state[`cg_${activeTab()}_s`]} min={0} max={100} disabled={props.bypassed} onChange={(v) => props.update(`cg_${activeTab()}_s`, Math.round(v))} trackBg={`linear-gradient(to right, #888888, hsl(${props.state[`cg_${activeTab()}_h`]}, 100%, 50%))`} />
        <Slider label="Luminance" value={props.state[`cg_${activeTab()}_l`]} min={-100} max={100} disabled={props.bypassed} onChange={(v) => props.update(`cg_${activeTab()}_l`, Math.round(v))} trackBg={`linear-gradient(to right, #000000, hsl(${props.state[`cg_${activeTab()}_h`]}, 100%, 50%), #ffffff)`} />
      </div>
    </div>
  );
};
...
const ColorMixerPanel: Component<{ state: any; bypassed: boolean; update: (field: string, val: number) => void; }> = (props) => {
  const [mode, setMode] = createSignal<"hsl" | "color">("hsl"); const [activeColor, setActiveColor] = createSignal<string>("r");
  return (
    <div style={{ padding: "12px 14px" }}>
      <style>{`
      ...
      {mode() === "color" && (
        <>
          <div style={{ display: "flex", "justify-content": "space-between", "margin-bottom": "16px", padding: "4px 0" }}>
            {MIXER_COLORS.map((c) => ( <div onClick={() => setActiveColor(c.id)} class={`color-mixer-square ${activeColor() === c.id ? "active" : ""}`} style={{ background: c.hex, opacity: props.bypassed ? 0.4 : 1 }} /> ))}
          </div>
          {MIXER_COLORS.map( (c) => activeColor() === c.id && (
                <div style={{ display: "flex", "flex-direction": "column", gap: "2px" }}>
                  ...
                  <Slider label="Saturation" value={props.state[`cm_s_${c.id}`]} disabled={props.bypassed} onChange={(v) => props.update(`cm_s_${c.id}`, v)} trackBg={`linear-gradient(to right, #888888, ${c.hex})`} />
                  <Slider label="Luminance" value={props.state[`cm_l_${c.id}`]} disabled={props.bypassed} onChange={(v) => props.update(`cm_l_${c.id}`, v)} trackBg={`linear-gradient(to right, #000000, ${c.hex}, #ffffff)`} />
                </div>
              )
          )}
        </>
      ...
      {mode() === "hsl" && (
        <div style={{ display: "flex", "flex-direction": "column", gap: "16px" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", "flex-direction": "column", gap: "2px" }}>
              {MIXER_COLORS.map((c) => ( <Slider label={c.label} value={props.state[`cm_s_${c.id}`]} disabled={props.bypassed} onChange={(v) => props.update(`cm_s_${c.id}`, v)} trackBg={`linear-gradient(to right, #888888, ${c.hex})`} /> ))}
            </div>
          </div>
          <div style={{ height: "1px", background: "#333" }}></div>
          <div>
            ...
            <div style={{ display: "flex", "flex-direction": "column", gap: "2px" }}>
              {MIXER_COLORS.map((c) => ( <Slider label={c.label} value={props.state[`cm_l_${c.id}`]} disabled={props.bypassed} onChange={(v) => props.update(`cm_l_${c.id}`, v)} trackBg={`linear-gradient(to right, #000000, ${c.hex}, #ffffff)`} /> ))}
            </div>
          </div>
        </div>
      )}
...
const App: Component = () => {
  const [lightState, setLightState] = createStore({
    ...
    crop_x: 0, crop_y: 0, crop_w: 1, crop_h: 1, crop_aspect: "free", crop_orientation: "landscape", is_cropping: false, crop_w_px: 0, crop_h_px: 0,
    enabled: true,
  });

  const [isWasmReady, setIsWasmReady] = createSignal(false);
  const [isCompare, setIsCompare] = createSignal(false);
  const [isOriginal, setIsOriginal] = createSignal(false);
  ...
  const [panelOrder, setPanelOrder] = createSignal<PanelId[]>([
    "histogram", "crop", "wb", "exposure", "hdr", "clarity", "dehaze", "vignette", "curve", "texture", "color_mixer", "color_grading", "bloom", "halation", "grain"
  ]);
  const [draggedIndex, setDraggedIndex] = createSignal<number | null>(null);
  const [expanded, setExpanded] = createStore<Record<PanelId, boolean>>({
    histogram: true, crop: false, wb: true, exposure: false, hdr: false, clarity: false, dehaze: false, curve: false, texture: false, halation: false, bloom: false, grain: false, color_grading: false, color_mixer: false, vignette: false,
  });
  const [bypassed, setBypassed] = createStore<Record<string, boolean>>({
    wb: false, exposure: false, hdr: false, clarity: false, dehaze: false, curve: false, texture: false, halation: false, bloom: false, grain: false, color_grading: false, color_mixer: false, vignette: false,
  });
  ...
  onMount(() => {
    ...
    if (typeof window.Module !== "undefined" && window.Module.ccall) { initBackend(); } else { window.addEventListener("wasm-ready", initBackend); onCleanup(() => window.removeEventListener("wasm-ready", initBackend)); }
  });
  ...
  const getProcessedLightState = () => ({
    ...
    hal_thresh: bypassed.halation ? 80 : lightState.hal_thresh, hal_radius: bypassed.halation ? 0 : lightState.hal_radius, hal_intensity: bypassed.halation ? 0 : lightState.hal_intensity,
    bloom_intensity: bypassed.bloom ? 0 : lightState.bloom_intensity,
    grain_amount: bypassed.grain ? 0 : lightState.grain_amount,
    cg_s_h: bypassed.color_grading ? 0 : lightState.cg_s_h, cg_s_s: bypassed.color_grading ? 0 : lightState.cg_s_s, cg_s_l: bypassed.color_grading ? 0 : lightState.cg_s_l, cg_m_h: bypassed.color_grading ? 0 : lightState.cg_m_h, cg_m_s: bypassed.color_grading ? 0 : lightState.cg_m_s, cg_m_l: bypassed.color_grading ? 0 : lightState.cg_m_l, cg_h_h: bypassed.color_grading ? 0 : lightState.cg_h_h, cg_h_s: bypassed.color_grading ? 0 : lightState.cg_h_s, cg_h_l: bypassed.color_grading ? 0 : lightState.cg_h_l, cg_g_h: bypassed.color_grading ? 0 : lightState.cg_g_h, cg_g_s: bypassed.color_grading ? 0 : lightState.cg_g_s, cg_g_l: bypassed.color_grading ? 0 : lightState.cg_g_l,
    cm_h_r: bypassed.color_mixer ? 0 : lightState.cm_h_r, cm_s_r: bypassed.color_mixer ? 0 : lightState.cm_s_r, cm_l_r: bypassed.color_mixer ? 0 : lightState.cm_l_r, cm_h_o: bypassed.color_mixer ? 0 : lightState.cm_h_o, cm_s_o: bypassed.color_mixer ? 0 : lightState.cm_s_o, cm_l_o: bypassed.color_mixer ? 0 : lightState.cm_l_o, cm_h_y: bypassed.color_mixer ? 0 : lightState.cm_h_y, cm_s_y: bypassed.color_mixer ? 0 : lightState.cm_s_y, cm_l_y: bypassed.color_mixer ? 0 : lightState.cm_l_y, cm_h_g: bypassed.color_mixer ? 0 : lightState.cm_h_g, cm_s_g: bypassed.color_mixer ? 0 : lightState.cm_s_g, cm_l_g: bypassed.color_mixer ? 0 : lightState.cm_l_g, cm_h_a: bypassed.color_mixer ? 0 : lightState.cm_h_a, cm_s_a: bypassed.color_mixer ? 0 : lightState.cm_s_a, cm_l_a: bypassed.color_mixer ? 0 : lightState.cm_l_a, cm_h_b: bypassed.color_mixer ? 0 : lightState.cm_h_b, cm_s_b: bypassed.color_mixer ? 0 : lightState.cm_s_b, cm_l_b: bypassed.color_mixer ? 0 : lightState.cm_l_b, cm_h_p: bypassed.color_mixer ? 0 : lightState.cm_h_p, cm_s_p: bypassed.color_mixer ? 0 : lightState.cm_s_p, cm_l_p: bypassed.color_mixer ? 0 : lightState.cm_l_p, cm_h_m: bypassed.color_mixer ? 0 : lightState.cm_h_m, cm_s_m: bypassed.color_mixer ? 0 : lightState.cm_s_m, cm_l_m: bypassed.color_mixer ? 0 : lightState.cm_l_m,
  ...
  const panelMeta: Record<PanelId, { title: string; features: boolean; reset: () => void }> = {
    ...
    crop: { title: "Crop & Rotate", features: false, reset: () => setLightState({ crop_x: 0, crop_y: 0, crop_w: 1, crop_h: 1, crop_aspect: "free", is_cropping: false }) },
  ...
  const renderContent = (id: PanelId) => {
    switch (id) {
      ...
      case "crop": return <CropPanel state={lightState} update={setLightState as any} />;
      case "wb": return <div style={{ padding: "16px 14px", display: "flex", "flex-direction": "column", gap: "2px" }}><Slider label="Temperature" value={lightState.temp} disabled={bypassed.wb} onChange={(v) => setLightState("temp", v)} trackBg="linear-gradient(to right, #3b82f6, #fbbf24)" /><Slider label="Tint" value={lightState.tint} disabled={bypassed.wb} onChange={(v) => setLightState("tint", v)} trackBg="linear-gradient(to right, #22c55e, #d946ef)" /></div>;
      case "exposure": return <div style={{ padding: "16px 14px", display: "flex", "flex-direction": "column", gap: "2px" }}><div onMouseEnter={() => setActiveSliderName("Exposure")} onMouseLeave={() => setActiveSliderName(null)}><Slider label="Exposure" value={lightState.exposure} disabled={bypassed.exposure} onChange={(v) => setLightState("exposure", v)} /></div><Slider label="Contrast" value={lightState.contrast} disabled={bypassed.exposure} onChange={(v) => setLightState("contrast", v)} /><Slider label="Saturation" value={lightState.saturation} disabled={bypassed.exposure} onChange={(v) => setLightState("saturation", v)} trackBg="linear-gradient(to right, #888888 0%, #888888 35%, #8b5cf6 50%, #3b82f6 65%, #10b981 75%, #eab308 85%, #ef4444 100%)" /><Slider label="Vibrance" value={lightState.vibrance} disabled={bypassed.exposure} onChange={(v) => setLightState("vibrance", v)} trackBg="linear-gradient(to right, #888888 0%, #888888 35%, #8b5cf6 50%, #3b82f6 65%, #10b981 75%, #eab308 85%, #ef4444 100%)" /></div>;
      case "hdr": return <div style={{ padding: "16px 14px", display: "flex", "flex-direction": "column", gap: "2px" }}><div onMouseEnter={() => setActiveSliderName("Highlights")} onMouseLeave={() => setActiveSliderName(null)}><Slider label="Highlights" value={lightState.highlights} disabled={bypassed.hdr} onChange={(v) => setLightState("highlights", v)} /></div><div onMouseEnter={() => setActiveSliderName("Shadows")} onMouseLeave={() => setActiveSliderName(null)}><Slider label="Shadows" value={lightState.shadows} disabled={bypassed.hdr} onChange={(v) => setLightState("shadows", v)} /></div><Slider label="Whites" value={lightState.whites} disabled={bypassed.hdr} onChange={(v) => setLightState("whites", v)} /><Slider label="Blacks" value={lightState.blacks} disabled={bypassed.hdr} onChange={(v) => setLightState("blacks", v)} /></div>;
      ...
      case "vignette": return <div style={{ padding: "16px 14px", display: "flex", "flex-direction": "column", gap: "2px" }}><Slider label="Amount" value={lightState.vig_amount} min={-100} max={100} disabled={bypassed.vignette} onChange={(v) => setLightState("vig_amount", v)} /><Slider label="Midpoint" value={lightState.vig_midpoint} min={0} max={100} disabled={bypassed.vignette} onChange={(v) => setLightState("vig_midpoint", v)} /><Slider label="Roundness" value={lightState.vig_roundness} min={-100} max={100} disabled={bypassed.vignette} onChange={(v) => setLightState("vig_roundness", v)} /><Slider label="Feather" value={lightState.vig_feather} min={0} max={100} disabled={bypassed.vignette} onChange={(v) => setLightState("vig_feather", v)} /></div>;
      ...
      case "halation": return <div style={{ padding: "16px 14px", display: "flex", "flex-direction": "column", gap: "2px" }}><div style={{ display: "flex", "align-items": "center", gap: "6px" }}><div style={{ flex: 1 }} onMouseEnter={() => setActiveSliderName("Threshold")} onMouseLeave={() => setActiveSliderName(null)}><Slider label="Threshold" value={lightState.hal_thresh} min={0} max={100} disabled={bypassed.halation} onChange={(v) => setLightState("hal_thresh", v)} /></div><button onClick={() => setLightState("show_hal_map", !lightState.show_hal_map)} disabled={bypassed.halation} title="Toggle Binary View" style={{ background: lightState.show_hal_map ? "#e0e0e0" : "#222", color: lightState.show_hal_map ? "#111" : "#aaa", border: "1px solid #444", "border-radius": "4px", padding: "0 6px", "font-size": "9px", "font-weight": "800", cursor: bypassed.halation ? "default" : "pointer", height: "20px", "margin-bottom": "6px", transition: "all 0.15s" }}>B/W</button></div><div onMouseEnter={() => setActiveSliderName("Gaussian Blur (px)")} onMouseLeave={() => setActiveSliderName(null)}><Slider label="Radius (px)" value={lightState.hal_radius} min={0} max={100} disabled={bypassed.halation} onChange={(v) => setLightState("hal_radius", v)} /></div><Slider label="Intensity" value={lightState.hal_intensity} min={0} max={100} disabled={bypassed.halation} onChange={(v) => setLightState("hal_intensity", v)} /><div style={{ display: "flex", "align-items": "center", "justify-content": "space-between", padding: "6px 0", "margin-top": "4px", opacity: bypassed.halation ? 0.4 : 1 }}><span style={{ "font-size": "10px", color: "#b0b0b0", "text-transform": "capitalize" }}>Halation Color</span><input type="color" value={lightState.hal_color} disabled={bypassed.halation} onInput={(e) => setLightState("hal_color", e.currentTarget.value) } style={{ background: "none", border: "1px solid #444", "border-radius": "4px", cursor: "pointer", height: "22px", width: "32px", padding: 0 }} /></div></div>;
      case "grain": return <div style={{ padding: "16px 14px", display: "flex", "flex-direction": "column", gap: "2px" }}><Slider label="Amount" value={lightState.grain_amount} min={0} max={100} disabled={bypassed.grain} onChange={(v) => setLightState("grain_amount", v)} /><Slider label="Size" value={lightState.grain_size} min={0} max={100} disabled={bypassed.grain} onChange={(v) => setLightState("grain_size", v)} /><Slider label="Roughness" value={lightState.grain_roughness} min={0} max={100} disabled={bypassed.grain} onChange={(v) => setLightState("grain_roughness", v)} /><Slider label="Color Variance" value={lightState.grain_color_variance} min={0} max={100} disabled={bypassed.grain} onChange={(v) => setLightState("grain_color_variance", v)} /></div>;
    }
  };
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <main style={{ flex: 1, background: "transparent" }}>
          <Viewport
            ...
            updateLightState={setLightState as any}
          ...
          <For each={panelOrder()}>
            {(id, index) => (
              <div onDragOver={(e) => { e.preventDefault(); const c = draggedIndex(); if (c !== null && c !== index()) { const o = [...panelOrder()]; o.splice(index(), 0, o.splice(c, 1)[0]); setPanelOrder(o); setDraggedIndex(index()); } }} style={{ opacity: draggedIndex() === index() ? 0.3 : 1 }}>
                <div style={{ display: "flex", "align-items": "center", "justify-content": "space-between", padding: "8px 14px", background: "#050505", "border-bottom": "5px solid #1a1a1a", cursor: "pointer" }} onClick={() => setExpanded(id, !expanded[id])}>
                  <div style={{ display: "flex", "align-items": "center", gap: "6px" }}>
                    <ChevronRight size={14} color="#888" style={{ transform: expanded[id] ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.15s ease" }} />
                    <span style={{ "font-size": "12.5px", color: "#AAAAAA" }}>{panelMeta[id].title}</span>
                  </div>
                  <div style={{ display: "flex", "align-items": "center", gap: "8px" }}>
                    {panelMeta[id].features && (
                      <>
                        <button onClick={(e) => { e.stopPropagation(); setBypassed(id, !bypassed[id]); }} style={{ background: "none", border: "none", color: bypassed[id] ? "#444" : "#aaa", cursor: "pointer", display: "flex", padding: 0 }}>{bypassed[id] ? <EyeOff size={14} /> : <Eye size={14} />}</button>
                        <button onClick={(e) => { e.stopPropagation(); panelMeta[id].reset(); }} style={{ background: "none", border: "none", color: "#777", cursor: "pointer", display: "flex", padding: 0 }}><RotateCcw size={12} /></button>
                      </>
                    )}
