...
export const Viewport: Component<ViewportProps> = (props) => {
  ...
  // Render Performance Governor States
  ...
  const initWebGPU = async (incomingBitmap: ImageBitmap) => {
    ...
    // Exactly 128 bytes mapping (32 floats)
  ...
  const renderFrame = (isExport = false) => {
    ...
    // Auto-Detect Active User Interaction (10X FPS LOD Governor)
    const now = performance.now();
    const isInteracting = (!isExport && (now - lastRenderTime < 100 || isDragging || isDraggingSplitter)) ? 1.0 : 0.0;
    lastRenderTime = now;
    ...
    if (canvasRef && containerRef) {
      ...
      if (containerWidth > 0) {
        const dividerX = containerWidth * splitPos(); const canvasCenterX = containerWidth / 2 + offset().x;
        const canvasDisplayWidth = (isExport ? imgBitmap.width : pWidth) * scale(); const canvasLeft = canvasCenterX - canvasDisplayWidth / 2;
    ...
    // EXPLICIT 32-FLOAT ARRAY FOR SHADER LOD & BITMAP OVERRIDE
    const paramsArray = new Float32Array([
      ...
      props.isCompare ? 1.0 : 0.0, shaderSplitPos, active ? p.hal_thresh : 80, 
      active ? p.hal_radius : 10, hr, hg, hb, active ? p.hal_intensity : 0,
      active ? p.bloom_intensity : 0, p.show_hal_map ? 1.0 : 0.0, isInteracting,
      0, 0, 0, 0, 0, 0, 0, 0 // 8 explicit padding floats
    ]);
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative', display: 'flex', 'align-items': 'center', 'justify-content': 'center', overflow: 'hidden', background: bgColor(), transition: 'background 0.2s', cursor: hasImage() ? (isDragging ? 'grabbing' : 'grab') : 'default' }} onWheel={(e:any) => { if (!hasImage()) return; e.preventDefault(); setScale(s => Math.max(0.01, s * Math.exp(-e.deltaY * 0.002))); renderFrame(false); }} onMouseDown={(e:any) => { if (!hasImage()) return; isDragging = true; lastX = e.clientX; lastY = e.clientY; renderFrame(false); }} onMouseMove={(e:any) => { if (isDraggingSplitter) { const rect = containerRef.getBoundingClientRect(); setSplitPos(Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width))); renderFrame(false); } else if (isDragging) { setOffset(o => ({ x: o.x + (e.clientX - lastX), y: o.y + (e.clientY - lastY) })); lastX = e.clientX; lastY = e.clientY; renderFrame(false); } else { handleMouseMove(e); } }} onMouseUp={() => { isDragging = false; }} onMouseLeave={() => { isDragging = false; props.onHoverLuminance(null); }}>
      ...
      {props.isCompare && hasImage() && (
        <div onPointerDown={(e) => { e.stopPropagation(); e.currentTarget.setPointerCapture(e.pointerId); isDraggingSplitter = true; renderFrame(false); }} onPointerMove={(e) => { if (isDraggingSplitter) { e.stopPropagation(); const rect = containerRef.getBoundingClientRect(); setSplitPos(Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width))); renderFrame(false); } }} onPointerUp={(e) => { e.stopPropagation(); e.currentTarget.releasePointerCapture(e.pointerId); isDraggingSplitter = false; }} style={{ position: 'absolute', top: 0, bottom: 0, left: `${splitPos() * 100}%`, width: '40px', 'margin-left': '-20px', cursor: 'ew-resize', 'z-index': 100, display: 'flex', 'align-items': 'center', 'justify-content': 'center', 'touch-action': 'none' }}>
