...
export interface ViewportProps {
  ...
  getImportFn: (importFn: () => void) => void;
}
...
export const Viewport: Component<ViewportProps> = (props) => {
  ...
  const renderFrame = () => {
    ...
    const paramsArray = new Float32Array([
      ...
      active ? p.texture : 0, active ? p.clarity : 0, active ? p.dehaze : 0, active ? p.temp : 0, active ? p.tint : 0, active ? p.vibrance : 0, active ? p.saturation : 0,
      props.isCompare ? 1.0 : 0.0, 0, 0
    ]);
    ...
    const passEncoder = commandEncoder.beginRenderPass({ colorAttachments: [{ view: context.getCurrentTexture().createView(), clearValue: { r: 0.1, g: 0.1, b: 0.1, a: 1.0 }, loadOp: 'clear', storeOp: 'store' }] });
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    let rx = (e.clientX - rect.left) / rect.width; let ry = (e.clientY - rect.top) / rect.height;

    if (rx < 0 || rx > 1 || ry < 0 || ry > 1) { props.onHoverLuminance(null); return; }

    const rot = rotation();
    if (rot === 90) { const tmp = rx; rx = ry; ry = 1 - tmp; } else if (rot === 180) { rx = 1 - rx; ry = 1 - ry; } else if (rot === 270) { const tmp = rx; rx = 1 - ry; ry = tmp; }
    ...
    try {
      ...
      if (props.lightState.enabled && (!props.isCompare || rx >= 0.5)) {
        const l = props.lightState;
        const t_val = l.temp / 100; const tint_val = l.tint / 100;
        r *= (1.0 + (t_val * 0.18)) * (1.0 + (tint_val * 0.08)); g *= (1.0 - (tint_val * 0.14)); b *= (1.0 - (t_val * 0.18)) * (1.0 + (tint_val * 0.08));
        const exp = Math.pow(2, l.exposure / 50); r *= exp; g *= exp; b *= exp;
        const c = (l.contrast / 100) + 1; r = (r - 0.5) * c + 0.5; g = (g - 0.5) * c + 0.5; b = (b - 0.5) * c + 0.5;
        const baseLuma = 0.299 * r + 0.587 * g + 0.114 * b;
        const sMask = 1.0 - Math.max(0, Math.min(1, baseLuma / 0.5)); const hMask = Math.max(0, Math.min(1, (baseLuma - 0.5) / 0.5));
        r += r * (l.shadows / 100) * sMask + r * (l.highlights / 100) * hMask; g += g * (l.shadows / 100) * sMask + g * (l.highlights / 100) * hMask; b += b * (l.shadows / 100) * sMask + b * (l.highlights / 100) * hMask;
        const w_p = 1.0 - (l.whites / 200); const b_p = 0.0 - (l.blacks / 200);
        r = (r - b_p) / (w_p - b_p); g = (g - b_p) / (w_p - b_p); b = (b - b_p) / (w_p - b_p);
      }

      if (props.curves && (!props.isCompare || rx >= 0.5)) {
  ...
  createEffect(() => {
    const deps = [props.lightState.enabled, props.lightState.exposure, props.lightState.contrast, props.lightState.highlights, props.lightState.shadows, props.lightState.whites, props.lightState.blacks, props.lightState.texture, props.lightState.clarity, props.lightState.dehaze, props.lightState.temp, props.lightState.tint, props.lightState.vibrance, props.lightState.saturation, props.curves, props.isCompare];
    renderFrame();
  });
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative', display: 'flex', 'align-items': 'center', 'justify-content': 'center', overflow: 'hidden', cursor: hasImage() ? (isDragging ? 'grabbing' : 'grab') : 'default' }} onWheel={(e:any) => { if (!hasImage()) return; e.preventDefault(); setScale(s => Math.max(0.01, s * Math.exp(-e.deltaY * 0.002))); }} onMouseDown={(e:any) => { if (!hasImage()) return; isDragging = true; lastX = e.clientX; lastY = e.clientY; }} onMouseMove={(e:any) => { if (isDragging) { setOffset(o => ({ x: o.x + (e.clientX - lastX), y: o.y + (e.clientY - lastY) })); lastX = e.clientX; lastY = e.clientY; } else { handleMouseMove(e); } }} onMouseUp={() => isDragging = false} onMouseLeave={() => { isDragging = false; props.onHoverLuminance(null); }}>
      ...
      {!hasImage() && (
        <div style={{ 'z-index': 2, color: '#444', 'font-size': '12px', 'font-weight': '600', 'letter-spacing': '1px' }}>
