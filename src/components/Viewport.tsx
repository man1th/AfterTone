...
export const Viewport: Component<ViewportProps> = (props) => {
  ...
  // GPU Readback Lock to prevent mapped-buffer crashes during rapid scrubbing
  ...
  const initWebGPU = async (imgBitmap: ImageBitmap) => {
    ...
    // V2: Increased uniform buffer size from 32 to 64 bytes
  ...
  const renderFrame = () => {
    ...
    // Explicitly mapping all 16 floats for the V2 buffer
    ...
    // The Fix: Only enqueue the buffer copy if we are not currently mapping it
    if (!isReadingHistogram) {
      commandEncoder.copyBufferToBuffer(histogramBuffer, 0, readbackBuffer, 0, 1024);
    }

    device.queue.submit([commandEncoder.finish()]);

    // The Fix: Only map the buffer if it is safely unmapped and not in progress
  ...
  const exportImage = () => {
    ...
    canvasRef.toBlob((blob) => {
      ...
      a.href = url; a.download = 'aftertone-export.jpg';
  ...
  createEffect(() => {
    // Explicitly track ALL V2 parameters
    const deps = [
      props.lightState.enabled, props.lightState.exposure, props.lightState.contrast, 
      props.lightState.highlights, props.lightState.shadows, props.lightState.whites, 
      props.lightState.blacks, props.lightState.texture, props.lightState.clarity, 
      props.lightState.dehaze, props.lightState.temp, props.lightState.tint, 
      props.lightState.vibrance, props.lightState.saturation
    ];
    renderFrame();
  });
    ...
    <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative', display: 'flex', 'align-items': 'center', 'justify-content': 'center', overflow: 'hidden', cursor: hasImage() ? (isDragging ? 'grabbing' : 'grab') : 'default' }} onWheel={onWheel} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} onMouseLeave={onMouseUp}>
      ...
      <canvas 
        ...
        style={{ 
          ...
          display: hasImage() ? 'block' : 'none', 'box-shadow': '0 10px 40px rgba(0,0,0,0.5)',
