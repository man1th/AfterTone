...
interface ToneCurveProps {
  curves: CurveState;
  setCurves: (curves: CurveState) => void;
  disabled?: boolean;
}
...
export const ToneCurve: Component<ToneCurveProps> = (props) => {
  ...
  const handleMouseDown = (e: MouseEvent) => {
    ...
    if (existingIndex !== -1) {
      ...
      channelCurves.push(newPoint);
      channelCurves.sort((a, b) => a.x - b.x);
      dragIndex = channelCurves.findIndex(pt => pt.x === newPoint.x);
  ...
  const handleGlobalMouseMove = (e: MouseEvent) => {
    ...
    if (dragIndex === 0) { channelCurves[dragIndex] = { x: 0, y: coords.y }; } 
    else if (dragIndex === channelCurves.length - 1) { channelCurves[dragIndex] = { x: 1, y: coords.y }; } 
    else {
      const minX = channelCurves[dragIndex - 1].x + 0.01;
      const maxX = channelCurves[dragIndex + 1].x - 0.01;
      channelCurves[dragIndex] = { x: Math.max(minX, Math.min(maxX, coords.x)), y: coords.y };
    }
  ...
  const handleDoubleClick = (e: MouseEvent, index: number) => {
    e.stopPropagation();
    if (props.disabled || index === 0 || index === props.curves[activeChannel()].length - 1) return;
    const channelCurves = props.curves[activeChannel()].filter((_, i) => i !== index);
    props.setCurves({ ...props.curves, [activeChannel()]: channelCurves });
  };
    ...
    <div style={{ padding: '0 4px', opacity: props.disabled ? 0.4 : 1 }}>
      <div style={{ display: 'flex', gap: '6px', 'margin-bottom': '14px' }}>
        {(['master', 'red', 'green', 'blue'] as CurveChannel[]).map(ch => (
          <button onClick={() => setActiveChannel(ch)} style={{ flex: 1, padding: '5px 0', background: activeChannel() === ch ? '#2a2a2a' : '#1e1e1e', border: `1px solid ${activeChannel() === ch ? '#444' : '#2a2a2a'}`, 'border-radius': '4px', color: activeChannel() === ch ? colors[ch] : '#888', cursor: 'pointer', 'font-size': '10px', 'font-weight': '700', 'text-transform': 'uppercase' }}>
      ...
      <div style={{ position: 'relative', width: '100%', 'aspect-ratio': '1/1', background: '#141414', border: '1px solid #2d2d2d', 'border-radius': '6px', overflow: 'hidden' }}>
        <svg ref={svgRef} viewBox="0 0 100 100" style={{ width: '100%', height: '100%', cursor: 'crosshair', overflow: 'visible' }} onMouseDown={handleMouseDown}>
          <line x1="25" y1="0" x2="25" y2="100" stroke="#222" stroke-width="0.5" />
          <line x1="50" y1="0" x2="50" y2="100" stroke="#2a2a2a" stroke-width="0.5" />
          <line x1="75" y1="0" x2="75" y2="100" stroke="#222" stroke-width="0.5" />
          <line y1="25" x1="0" y2="25" x2="100" stroke="#222" stroke-width="0.5" />
