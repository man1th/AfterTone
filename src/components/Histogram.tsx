...
export const Histogram: Component<HistogramProps> = (props) => {
  ...
  createEffect(() => {
    ...
    const rData = props.data.slice(0, 256);
    const gData = props.data.slice(256, 512);
    const bData = props.data.slice(512, 768);
    const lData = props.data.slice(768, 1024);

    // Find local max avoiding single-frequency noise spikes
    ...
    <div style={{ width: '100%' }}>
      <div style={{ width: '100%', height: '110px', background: '#141414', border: '1px solid #282828', 'border-radius': '4px', position: 'relative', overflow: 'hidden' }}>
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" style={{ width: '100%', height: '100%' }}>
          ...
          <path d={paths().l} fill="rgba(75, 75, 75, 0.45)" stroke="none" />
          
          {/* Individual RGB Component Paths */}
          <path d={paths().r} fill="none" stroke="#ff4b4b" stroke-width="1" opacity="0.85" style={{ 'mix-blend-mode': 'screen' }} />
          <path d={paths().g} fill="none" stroke="#4ade80" stroke-width="1" opacity="0.85" style={{ 'mix-blend-mode': 'screen' }} />
          <path d={paths().b} fill="none" stroke="#60a5fa" stroke-width="1" opacity="0.85" style={{ 'mix-blend-mode': 'screen' }} />
          
          {/* Real-time Tracking Reticle Line */}
          {props.hoverLuminance !== null && (
            <line x1={props.hoverLuminance * 100} y1="0" x2={props.hoverLuminance * 100} y2="100" stroke="#D97757" stroke-width="1.25" />
          )}
        </svg>
      </div>
