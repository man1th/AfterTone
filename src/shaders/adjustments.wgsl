...
fn vs_main(@builtin(vertex_index) vertexIndex: u32) -> VertexOutput {
    ...
    var output: VertexOutput; output.position = vec4<f32>(pos[vertexIndex], 0.0, 1.0); output.uv = uv[vertexIndex];
    return output;
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if (params.texture_adj != 0.0 || params.clarity != 0.0 || params.dehaze != 0.0) {
        ...
        for (var i = 0; i < 8; i++) {
            ...
            let l = getLuma(samp); let diff = center_luma - l; let w = exp(-(diff * diff) / denom);
            mid_blur += samp * w; mid_w += w;
        }
        ...
        for (var i = 0; i < 8; i++) {
            ...
            spatial_min_dark = min(spatial_min_dark, min(min(samp.r, samp.g), samp.b));
            coarse_blur += samp; coarse_w += 1.0;
        }
